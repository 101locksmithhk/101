#!/usr/bin/env python3
"""Build the static site with directory-based, extension-free URLs.

The source HTML files keep their ``.html`` names so they still work when opened
locally.  The deployment output changes ``service.html`` into
``service/index.html`` and rewrites internal links to ``/service/``.
"""
...
def discover_source_pages() -> tuple[list[Path], dict[Path, Path]]:
    """Return canonical source pages and duplicate-index aliases."""
    all_pages = sorted(
        path.resolve()
    ...
    canonical_pages: list[Path] = []

    for page in all_pages:
        relative = page.relative_to(PROJECT_ROOT)
        sibling_page = PROJECT_ROOT / f"{relative.parent.as_posix()}.html"

        if (
            page.name == "index.html"
            and relative.parent != Path(".")
            and sibling_page.exists()
        ):
            aliases[page] = sibling_page.resolve()
            continue

        canonical_pages.append(page)

    return canonical_pages, aliases
...
class SiteBuilder:
    def __init__(self, output_root: Path, base_path: str) -> None:
        ...
        if len(output_paths) != len(set(output_paths)):
            raise RuntimeError("Two source pages resolve to the same clean URL.")
    ...
    def rewrite_language_targets(self, text: str) -> str:
        language_target = (
            ...
            f"? '{self.site_url('/zh-CN/')}' "
            f": (lang === 'en' ? '{self.site_url('/en/')}' : '{self.site_url('/')}');"
        )
...
def main() -> None:
    ...
    builder.build()
    print(
        f"Built {len(builder.pages)} clean-URL pages in {builder.output_root} "
